...
insert = "insert into board (seq, title, writer, content) values ((select NVL(max(seq),0)+1 from board), ?, ?, ?)";

update = "update board set title=?,content=? where seq =?";

delete = "delete from board where seq  = ? ";

get = "select * from board where seq  = ? ";

getList = "select * from board order by seq DESC";
